...
class RIPGeneric(JsonRpcServer):
  ...
  def __init__(self, name='RIP Server', description='Generic RIP Server Implementation.', authors='', keywords=''):
    '''
    Constructor
    '''
  ...
  def build_info(self):
    ...
    readables = RIPVariablesList(
      ...
      methods=[self.buildSSEGetInfo(), self.buildPOSTGetInfo()],
      read_notwrite=True
    )
    writables = RIPVariablesList(
      list_=self.writables,
      methods=[self.buildPOSTSetInfo()],
      read_notwrite=False
    )
    meta = RIPMetadata(info, readables, writables)
    return str(meta)

  def buildSSEGetInfo(self):
    return RIPMethod(
